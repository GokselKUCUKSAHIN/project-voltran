...
eventEmitter.on('done', (arg) => {

});


async function multiCoreLoadPool(sampleSize: number = 64, scaler: number = 100_000): Promise<number> {
    const resultArray = await Promise.all(multiCoreLoadPromiseArray(sampleSize, scaler));
    let i = 0;
    executeWorkerPromise(taskPath, {start: i * scaler, end: (i + 1) * scaler})

    return sum(resultArray);
}

async function cpuMultiCoreScorePool(): Promise<number> {
    const startTime = Date.now();
    await multiCoreLoadPool();
    const deltaTime = Date.now() - startTime;
    return 15_000_000 / deltaTime;
}

export async function cpuMultiCoreBenchPool(): Promise<number> {
    return parseFloat((await cpuMultiCoreScorePool()).toFixed(4));
}